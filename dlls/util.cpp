...
void UTIL_GunshotDecalTrace(TraceResult* pTrace, int decalNumber, bool bClientOnly, entvars_t* pShooter)
{
    if (decalNumber < 0)
        return;
    ...
    if (bClientOnly)
        MESSAGE_BEGIN(MSG_ONE, SVC_TEMPENTITY, pTrace->vecEndPos, ENT(pShooter));
    else
...
char UTIL_TextureHit(TraceResult* ptr, Vector vecSrc, Vector vecEnd)
{
    ...
    CBaseEntity* pEntity = (CBaseEntity*)GET_PRIVATE(pHit);
