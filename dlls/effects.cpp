...
void CBombGlow::Think(void)
{
    ...
    if (gpGlobals->time > m_lastTime + m_tmBeepPeriod)
    {
        ...
        if (m_tmBeepPeriod < 0.1)
            m_tmBeepPeriod = 0.1;

        pev->effects = 0;
        EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "weapons/c4_beep1.wav", VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
    }
...
void CTestEffect::TestThink(void)
{
    ...
    if (m_iBeam < 24)
    {
        ...
        WRITE_COORD(vecMid.x);
        WRITE_COORD(vecMid.y);
        WRITE_COORD(vecMid.z);
        WRITE_BYTE(20);
        WRITE_BYTE(255);
        WRITE_BYTE(180);
        WRITE_BYTE(100);
