...
EHANDLE CLocalNav::m_hQueue[MAX_HOSTAGES_NAV];
EHANDLE CLocalNav::m_hHostages[MAX_HOSTAGES_NAV];
...
float CLocalNav::m_flStepSize = 18.0f;
...
CLocalNav::CLocalNav(CHostage* pOwner)
{
    ...
    {
        if ((CBaseEntity*)m_hHostages[i] != NULL)
        {
            if (i != validCount)
                m_hHostages[validCount] = m_hHostages[i];
            validCount++;
        }
    }
    m_NumHostages = validCount;
...
node_index_t CLocalNav::GetBestNode(Vector& vecOrigin, Vector& vecDest)
{
    ...
    while (nindexCurrent < m_nindexAvailableNode)
    {
        ...
        if (!nodeCurrent->fSearched)
        {
            ...
            flDistFromStart = (vecDest - nodeCurrent->vecLoc).Length();
...
PathTraversAble CLocalNav::PathTraversable(Vector& vecSource, Vector& vecDest, BOOL fNoMonsters)
{
    ...
    Vector vecDir;
    float flTotal;
    PathTraversAble retval = PTRAVELS_EMPTY;
...
BOOL CLocalNav::SlopeTraversable(Vector& vecSource, Vector& vecDest, BOOL fNoMonsters, TraceResult& tr)
{
    ...
    vecSlopeEnd.z = vecDown.Length2D() * tan((90.0 - vecAngles.x) * (M_PI / 180)) + vecSource.z;
...
void CLocalNav::Think()
{
    ...
    {
        m_flStepSize = CVAR_GET_FLOAT("sv_stepsize");
    ...
    m_NodeValue -= (int)(flElapsedTime * 250);
    ...
    if (m_NumRequest)
    {
        ...
        EHANDLE hHostage = m_hQueue[m_CurRequest];
        while ((CBaseEntity*)hHostage == NULL && m_NumRequest > 0)
        {
        ...
        pHostage = (CHostage*)(CBaseEntity*)hHostage;
...
void CLocalNav::RequestNav(CHostage* pCaller)
{
    ...
    {
        if ((CBaseEntity*)m_hQueue[curr] == (CBaseEntity*)pCaller)
            return;
...
void CLocalNav::HostagePrethink()
{
    for (int i = 0; i < m_NumHostages; i++)
    {
        CHostage* pHostage = (CHostage*)(CBaseEntity*)m_hHostages[i];
        if (pHostage != NULL)
        {
            pHostage->PreThink();
        }
